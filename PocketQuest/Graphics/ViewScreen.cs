...
namespace Graphics3D
{
    public class ViewScreen : IGameObject3D
    {
        ...
        private PointR3 INIT_POSITION = new PointR3(0, 0, 1.6);         // center of the view screen
        private PointR3 DIST_FROM_SCREEN = new PointR3(-0.7, 0, 0.1);   // where the viewer's eye is
        ...
        public PlaneR3 GetViewPlane()
        {
            ...
            return new PlaneR3(screenOrigin, upperRightCorner, lowerLeftCorner);
        }
        ...
        public VectorR3 GetViewDirection() { return GetIHat().Cross(GetJHat()); }
        // returns the position of the viewer
        public PointR3 GetEyePosition() { return positionBundle[EYE].ToPoint(); }
        // for rotating and moving the view screen, required by IGameObject3D parent class
        public VectorR3[] GetVertices() { return positionBundle; }
        public void SetVertex(int index, VectorR3 vertex) { positionBundle[index] = vertex; }
        // private methods for constructor intialization...
        private PointR3 GeneratePixelOffsetPoint()
        {
            PointR3 offset = new PointR3();

            return offset.Add(INIT_POSITION).Add(SCREEN_HEIGHT.Sx(0.5)).Add(SCREEN_WIDTH.Sx(0.5));
        }
        private PointR3 GenerateEyePoint() { return DIST_FROM_SCREEN.Add(INIT_POSITION); }
        private VectorR3[] GeneratePositionBundle()
        {
            ...
            bundle[LL_CORNER] = bundle[PIXEL_OFFSET].Sub(SCREEN_HEIGHT.ToVector());
            bundle[UR_CORNER] = bundle[PIXEL_OFFSET].Sub(SCREEN_WIDTH.ToVector());

            return bundle;
        }
        // private methods for geting screen vetors i hat and j hat
        private VectorR3 GetIHat() { return positionBundle[UR_CORNER].Sub(positionBundle[PIXEL_OFFSET]); }
        private VectorR3 GetJHat() { return positionBundle[LL_CORNER].Sub(positionBundle[PIXEL_OFFSET]); }
        // private methods for the physical dimensions of each pixel
        private double GetWidthOfPixel() { return SCREEN_WIDTH.Get(CoordinateR3.Y) / width; }
        private double GetHeightOfPixel() { return SCREEN_HEIGHT.Get(CoordinateR3.Z) / height; }
        // upper left hand corner of the screen or (0,0) relative to screens coorddinates (i,j)
        private PointR3 GetPixelOffset() { return new PointR3(positionBundle[PIXEL_OFFSET]); }
    }
}
